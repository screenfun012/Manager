...
const initDatabase = () => {
  db.serialize(() => {
    ...
    db.run(`CREATE TABLE IF NOT EXISTS assignments (
      ...
      date TEXT NOT NULL,
      original_quantity INTEGER DEFAULT NULL,
      modified_quantity INTEGER DEFAULT NULL,
    ...
    // Migracija za nova polja u assignments tabeli
    db.run(`ALTER TABLE assignments ADD COLUMN original_quantity INTEGER DEFAULT NULL`);
    db.run(`ALTER TABLE assignments ADD COLUMN modified_quantity INTEGER DEFAULT NULL`);
    db.run(`ALTER TABLE assignments ADD COLUMN is_modified INTEGER DEFAULT 0`);
    db.run(`ALTER TABLE assignments ADD COLUMN modified_at DATETIME DEFAULT NULL`);
...
const loadSampleData = () => {
  ...
  // Ažuriranje postojećih assignments da se popune nova polja
  db.run(`UPDATE assignments SET original_quantity = quantity WHERE original_quantity IS NULL`, (err) => {
    if (err) {
      console.log('ℹ️ Polja već postoje ili nema podataka za ažuriranje');
    } else {
      console.log('✅ Ažurirana polja za praćenje izmena');
    }
  });
};
